...
class AddStationMeasures(object):
    ...
    def __init__(
            ...
            field_map: Dict[str, str],                  # The map between the API fields and database param code.
    ...
    def process(self, datamodels: ThingspeakAPIDataBuilder):
        print(f"found #{len(datamodels)} datamodels")
        requests = AddThingspeakMeasuresRequestBuilder(datamodel=datamodels, code2id=self.code2id, field_map=self.field_map)
        ...
        responses = AddStationMeasuresResponseBuilder(
            requests=valid_requests, start_packet_id=self.start_packet_id, sensor_id=self.sensor_id
        )
        if responses:
            ...
            self.output_gateway.update_last_acquisition(
                timestamp=last_acquisition, sensor_id=self.sensor_id, ch_name=self.ch_name
            )
