...
class SelectWrapper(baseop.DatabaseWrapper, abc.ABC):
    ...
    def _select_api_param(self, sensor_id: int) -> List[chtype.Channel]:
        ...
        return [chtype.Channel(ch_id=ident, ch_key=key, ch_name=name, last_acquisition=ts.datetime2timestamp(datetime_=timest))
                for key, ident, name, timest in api_param_resp]
    ...
    def select_measure_param(self) -> Dict[str, int]:
        ...
        return {param_code: param_id for param_code, param_id in measure_param_resp}
