...
################################ INFO DATABASE RESPONSE ################################
class InfoDatabaseResponse:

    def __init__(self, sensor_id: int, sensor_name: str, api_param: List[chtype.Channel],
                 geometry: pgis.PostgisGeometry):
...
################################ SENSOR INFO SELECT WRAPPER ################################
class SensorInfoSelectWrapper(base.SelectWrapper):

    def __init__(
            self,
            conn: db.DatabaseAdapter,
            builder: qry.QueryBuilder,
            sensor_type: str,
            pgis_cls=pgis.PostgisPoint,
            log_filename="log"
    ):
