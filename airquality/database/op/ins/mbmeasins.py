...
class MobileMeasureInsertWrapper(ins.InsertWrapper):
    ...
    def __init__(self, conn: db.DatabaseAdapter, query_builder: qry.QueryBuilder, log_filename="log"):
        super(MobileMeasureInsertWrapper, self).__init__(conn=conn, query_builder=query_builder, log_filename=log_filename)

    @log_decorator.log_decorator()
    def concat_mobile_measurements_query(self, records: List[mbrec.MobileMeasureRecord]) -> None:

        self.query_to_execute += self.builder.insert_mobile_measurements(records=records)
