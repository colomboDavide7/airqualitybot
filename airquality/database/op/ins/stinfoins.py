...
class StationInfoInsertWrapper(base.InsertWrapper):
    ...
    def __init__(self, conn: connection.DatabaseAdapter, query_builder: query.QueryBuilder, log_filename="log"):
        super(StationInfoInsertWrapper, self).__init__(conn=conn, query_builder=query_builder, log_filename=log_filename)

    @log_decorator.log_decorator()
    def concat_initialize_sensor_query(self, records: List[rec.StationInfoRecord]) -> None:

        self.query_to_execute += self.builder.initialize_sensors(records=records)
