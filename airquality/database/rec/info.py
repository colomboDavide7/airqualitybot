...
class InfoRecordBuilder(log.Loggable):
    ...
    def get_sensor_value(self, sensor_name: str, sensor_type: str) -> str:
        return f"({self._sensor_id}, '{sensor_type}', '{sensor_name}'),"

    def get_api_param_values(self, ident: str, key: str, name: str, timest: ts.Timestamp) -> str:
        return f"({self._sensor_id}, '{key}', '{ident}', '{name}', '{timest.get_formatted_timestamp()}')"

    def get_geolocation_value(self, timest: ts.Timestamp, geometry: pgis.PostgisGeometry) -> str:
        return f"({self._sensor_id}, '{timest.get_formatted_timestamp()}', {geometry.geom_from_text()}),"
