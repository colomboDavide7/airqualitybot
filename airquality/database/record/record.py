...
import container.sensor.sensor as sens_c


class SensorContainerSQLAdapter:

    def __init__(self, sensor_container: sens_c.SensorContainer):
        self.sensor_container = sensor_container

    def api_param_to_sql(self):
        record = ""
        sensor_id = self.sensor_container.identity.id
        for channel in self.sensor_container.channels:
            ident = channel.api_param_container.ident
            key = channel.api_param_container.key
            record += f"({sensor_id}, '{ident.name}', '{ident.value}'),({sensor_id}, '{key.name}', '{key.value}'),"
        return record.strip(',')

    def sensor_to_sql(self):
        return f"('{self.sensor_container.identity.type}', '{self.sensor_container.identity.name}')"

    def channel_info_to_sql(self):
        record = ""
        sensor_id = self.sensor_container.identity.id
        for channel in self.sensor_container.channels:
            info = channel.channel_info_container
            record += f"({sensor_id}, '{info.name}', '{info.last_acquisition.get_formatted_timestamp()}'),"
        return record.strip(',')

    def location_to_sql(self):
        sensor_id = self.sensor_container.identity.id
        location = self.sensor_container.location
        return f"({sensor_id}, '{location.valid_from.get_formatted_timestamp()}', {location.postgis_geom.geom_from_text()})"
