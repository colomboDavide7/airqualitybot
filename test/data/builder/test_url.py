...
class TestURLBuilder(unittest.TestCase):
    ...
    def test_successfully_build_purpleair_url(self):
        ...
        purpleair_builder = PurpleairURLBuilder(api_address=self.api_address, parameters=test_param)
    ...
    def test_system_exit_when_building_purpleair_url(self):
        ...
        with self.assertRaises(SystemExit):
            PurpleairURLBuilder(api_address=self.api_address, parameters=test_param_missing_key)

        test_param_missing_fields = {"api_key": "some_key", "opt": "val"}
        with self.assertRaises(SystemExit):
            PurpleairURLBuilder(api_address=self.api_address, parameters=test_param_missing_fields)

        test_param_empty_fields = {"api_key": "some_key", "fields": [], "opt": "val"}
        builder = PurpleairURLBuilder(api_address=self.api_address, parameters=test_param_empty_fields)
        with self.assertRaises(SystemExit):
            builder.url()
