...
class TestURLBuilder(unittest.TestCase):
    ...
    def test_successfully_build_purpleair_url(self):
        ...
        purpleair_builder = url.PurpleairURLBuilder(address=self.address, url_param=test_param)
        actual_output = purpleair_builder.dyn()
        self.assertEqual(actual_output, expected_output)

    def test_exit_on_missing_purpleair_api_key(self):
        test_param_missing_key = {"fields": ["f1", "f2"], "opt": "val"}
        url_builder = url.PurpleairURLBuilder(address=self.address, url_param=test_param_missing_key)
        with self.assertRaises(SystemExit):
            url_builder.dyn()

    def test_exit_on_missing_purpleair_fields(self):
        test_param_missing_fields = {"api_key": "some_key", "opt": "val"}
        url_builder = url.PurpleairURLBuilder(address=self.address, url_param=test_param_missing_fields)
        with self.assertRaises(SystemExit):
            url_builder.dyn()

    def test_exit_on_empty_fields(self):
        test_param_empty_fields = {"api_key": "some_key", "fields": [], "opt": "val"}
        builder = url.PurpleairURLBuilder(address=self.address, url_param=test_param_empty_fields)
        with self.assertRaises(SystemExit):
            builder.dyn()
    ...
    def test_successfully_build_atmotube_url(self):
        ...
        atmotube_builder = url.AtmotubeURLBuilder(address=self.address, url_param=test_param)
        actual_output = atmotube_builder.dyn()
        self.assertEqual(actual_output, expected_output)

    def test_exit_on_missing_atmotube_api_key(self):
        test_param_missing_key = {'mac': 'some_mac', "opt": "val"}
        url_builder = url.AtmotubeURLBuilder(address=self.address, url_param=test_param_missing_key)
        with self.assertRaises(SystemExit):
            url_builder.dyn()

    def test_exit_on_missing_atmotube_mac_address(self):
        test_param_missing_mac = {"api_key": "key", "opt": "val"}
        url_builder = url.AtmotubeURLBuilder(address=self.address, url_param=test_param_missing_mac)
        with self.assertRaises(SystemExit):
            url_builder.dyn()
    ...
    def test_successfully_build_thingspeak_url(self):
        ...
        thingspeak_builder = url.ThingspeakURLBuilder(address=self.address, url_param=test_param)
        actual_output = thingspeak_builder.dyn()
        self.assertEqual(actual_output, expected_output)

    def test_exit_on_missing_thingspeak_channel_id(self):
        test_param_missing_id = {'format': 'json', "api_key": "key", "opt": "val"}
        url_builder = url.ThingspeakURLBuilder(address=self.address, url_param=test_param_missing_id)
        with self.assertRaises(SystemExit):
            url_builder.dyn()
    ...
    def test_exit_on_missing_thingspeak_response_format(self):
        ...
        with self.assertRaises(SystemExit):
            url.ThingspeakURLBuilder(address=self.address, url_param=test_param_missing_format)

    def test_exit_on_missing_thingspeak_channel_key(self):
        test_param_missing_key = {"channel_id": "id", 'format': 'json', "opt": "val"}
        url_builder = url.ThingspeakURLBuilder(address=self.address, url_param=test_param_missing_key)
        with self.assertRaises(SystemExit):
            url_builder.dyn()
